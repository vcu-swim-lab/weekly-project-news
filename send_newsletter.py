...
def draft_email(subject, content):
    ...
    if response.status_code >= 200 and response.status_code < 300:
        ...
        print(f"Successfully drafted email with subject: {subject}. Printing response")
        print(response_data)
...
def send_email_to_subscriber(subscriber_id, email_id):
    print("Starting sending email process")
    ...
    if response.status_code >= 200 and response.status_code < 300:
        ...
        if update_response.status_code >= 200 and update_response.status_code < 300:
            ...
            print(f"Successfully sent email (ID: {email_id}) to subscriber (ID: {subscriber_id})")
            return response
        else:
...
def update_email_status(email_id):
    print("Starting updating email process")
...
for subscriber in subscribers_data['results']:
    ...
    # For pushing to server, uncomment this block
    ...
    subject = f"Weekly GitHub Report for {capitalized_name}: {timestamp_from} - {timestamp_to} for ({email})"
    ...
    print(f"Printing draft: {response}")
    if not response:
        print(f"Draft email not able to be made for: {email}")
        logging.error(f"Draft email not able to be made for: {email}, no response")
        continue

    # STEP 5: SEND the email to the subscriber
    email_id = response['id']
    subscriber_id = subscriber['id']
    print(f"Sending email {email_id} to subscriber {subscriber_id} ({email})")

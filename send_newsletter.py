...
# Build the email subject for a repo's weekly report. Kept as a small pure
# function so it can be unit tested and so an empty repo name can't crash the
# send loop.
def build_subject(github_repo, now=None):
    if now is None:
        now = datetime.now()
    name = github_repo.rstrip('/').split('/')[-1] if github_repo else ''
    # Capitalize only the first character, preserving the rest (e.g.
    # "llama.cpp" -> "Llama.cpp"). name[:1] is empty-safe, unlike name[0].
    capitalized_name = name[:1].upper() + name[1:]
    timestamp = now.strftime('%H:%M:%S')
    timestamp_from = (now - timedelta(days=7)).strftime('%B %d, %Y')
    timestamp_to = now.strftime('%B %d, %Y')
    return f"Weekly GitHub Report for {capitalized_name}: {timestamp_from} - {timestamp_to} ({timestamp})"
...
def main():
    ...
    for subscriber in subscribers_data['results']:
        ...
        # Check if we've already processed this email address. This is the only
        # dedup that matters: one email address should receive a repo's report
        # once. (We must NOT dedup on subject — multiple subscribers to the same
        # repo legitimately share an identical subject and must each be sent.)
        ...
        subject = build_subject(github_repo)
        ...
        # Record this email's metadata (used for the end-of-run summary)
